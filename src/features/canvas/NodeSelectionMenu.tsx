...
export function NodeSelectionMenu({
  ...
  return (
    <div
      ref={menuRef}
      className={`
        absolute z-50 min-w-[220px] overflow-hidden rounded-lg border border-[var(--canvas-node-border)] bg-[var(--canvas-menu-bg)] shadow-xl
    ...
    >
      {menuItems.map((item, index) => {
        ...
        return (
          <button
            key={item.type}
            className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--canvas-menu-item-hover)]"
          ...
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-md bg-[var(--canvas-node-section-bg)]">
              <Icon className="h-4 w-4 text-accent" />
            </div>
            <span className="text-sm text-[var(--canvas-node-fg)]">{t(item.menuLabelKey)}</span>
          </button>
        );
      })}
    </div>
