# System Design Plan: Storyboard Copilot Web

## Context

Storyboard Copilot 是一款 AI 驱动的分镜创作工具（图片生成/编辑、视频生成、节点画布）。现有桌面版（Tauri 2，仓库 `Storyboard-Copilot`）已验证核心工作流，现升级扩展为 Web SaaS 产品以实现更广泛的用户触达、多用户支持和商业化（覆盖全球和中国市场）。

**仓库策略**：桌面版仓库 `Storyboard-Copilot` 保留不变。Web 版在独立新仓库 `storyboard-copilot-web` 中开发。
